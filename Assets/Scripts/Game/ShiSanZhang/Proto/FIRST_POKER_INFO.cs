...
public class FIRST_POKER_INFO { 
	...
	public static FIRST_POKER_INFO newBuilder() { 
		return new FIRST_POKER_INFO(); 
	} 

	public static FIRST_POKER_INFO decode(byte[] data) { 
		FIRST_POKER_INFO proto = newBuilder();
		proto.build(data);
		return proto; 
	} 
	...
	public byte[] encode() { 
		...
		if(this.hasType()) {
			...
			bytes[3].putInt(this.type);
			total += bytes[3].limit();
		}

		if(this.hasPockerStatus()) {
			bytes[4] = ByteBuffer.allocate(4);
			bytes[4].putInt(this.pocker_status);
			total += bytes[4].limit();
		}
